...
class SAFacebookhooks{
    ...
    public static function ob_facebook(&$buffer){
	    ...
	    $loginUrl = $facebook->getLoginUrl(
            array(
		        ...
		        'scope' => 'offline_access,email,status_update,publish_stream'
		    )
	    );
    ...
    public static function face_init(){
        ...
        $Url = $facebook->getLoginUrl(array('redirect_uri' => $scripturl.'?action=facebook','scope' => 'offline_access,email,status_update,publish_stream'));
        $loginUrl = $facebook->getLoginUrl(array('redirect_uri' => $scripturl.'?action=facebookintegrate','scope' => 'offline_access,email,status_update,publish_stream'));
	    $profileUrl = $facebook->getLoginUrl(array('redirect_uri' => $scripturl.'?action=profile','scope' => 'offline_access,email,status_update,publish_stream'));
        ...
        if($user){
			...
			try {
                ...
                if(empty($_SESSION['safbusername1']) || empty($_SESSION['safbpic']) || empty($_SESSION['safbuserid']) || empty($_SESSION['safbpemail']) || empty($_SESSION['safbname']) || empty($_SESSION['safbusername'])){
					...
					$_SESSION['safbusername1'] = !empty($user_profile['username']) ? $user_profile['username'] : '';
